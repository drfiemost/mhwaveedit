...
static const char * D08_contents[] = {
     "\n",
     /* xgettext:no-c-format */ N_("mhWaveEdit creates a directory ~/.config where it stores configuration information. \n"),
     "\n",
     ...
     "\n",
     /* xgettext:no-c-format */ N_("Each mhwaveedit process creates a session file in the .cache directory called mhwaveedit-session-<pid>-<session>-<state>, where <session> is the session ID number and <state> is a character code showing the state of the session ('r' for running sessions). \n"),
     "\n",
     /* xgettext:no-c-format */ N_("Temporary files are by default also stored in the ~/.cache directory. Which directories to use can be set through the preferences dialog. To get the best performance, you should have one temporary directory for each local filesystem. The temporary files have names of the form \"mhwaveedit-temp-<pid>-nnnn-<session>\". Do NOT open or remove temporary files with the same pid number as a currently running mhWaveEdit.\n"),
     "\n",
